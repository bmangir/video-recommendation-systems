...
CREATE INDEX idx_likes_video ON user_likes(video_id);
CREATE INDEX idx_likes_type ON user_likes(video_id, like_type);

-- Partial index for quick like counts
CREATE INDEX idx_likes_only ON user_likes(video_id) WHERE like_type = 1;
CREATE INDEX idx_dislikes_only ON user_likes(video_id) WHERE like_type = -1;
