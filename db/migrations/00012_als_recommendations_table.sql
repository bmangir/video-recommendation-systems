...
DROP TABLE IF EXISTS als_recommendations CASCADE;
...
CREATE INDEX idx_als_user_latest ON als_recommendations(user_id, generated_at DESC)
    WHERE is_active = true;

-- Get all recommendations generated by a specific model version
CREATE INDEX idx_als_model_version ON als_recommendations(model_version, generated_at DESC);

-- Cleanup old recommendations
CREATE INDEX idx_als_cleanup ON als_recommendations(is_active, generated_at) WHERE is_active = false;

CREATE INDEX idx_als_video_recommendations ON als_recommendations USING GIN(recommended_video_ids);
...
COMMENT ON TABLE als_recommendations IS 'User-specific video recommendations generated by ALS collaborative filtering.';
COMMENT ON COLUMN als_recommendations.recommended_video_ids IS 'Ordered array of video IDs (best first)';
COMMENT ON COLUMN als_recommendations.scores IS 'Confidence scores for each video (parallel to video_ids)';
COMMENT ON COLUMN als_recommendations.model_version IS 'Format: als_vX.Y_YYYYMMDD';
